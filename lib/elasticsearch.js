...
var es_bulk_insert = function elasticsearch_bulk_insert(listCounters, listTimers, listTimerData, listGaugeData) {
      ...
      for (key in listCounters) {
        ...
        payload += '{';
        innerPayload = '"type":"'+elasticCountType+'",';
          for (statKey in listCounters[key]){
            if (innerPayload) innerPayload += ',';
            innerPayload += renderKV(statKey, listCounters[key][statKey], elasticCountType);
          }
      ...
      for (key in listTimers) {
        payload += '{"index":{"_index":"'+statsdIndex+'","_type":"doc"}}'+"\n";
        payload += '{';
        innerPayload = '"type":"'+elasticTimerType+'",';
          for (statKey in listTimers[key]){
            if (innerPayload) innerPayload += ',';
            innerPayload += renderKV(statKey, listTimers[key][statKey], elasticTimerType);
          }
        payload += innerPayload +'}'+"\n";
      }
      for (key in listTimerData) {
        payload += '{"index":{"_index":"'+statsdIndex+'","_type":"doc"}}'+"\n";
        payload += '{';
        innerPayload = '"type":"'+elasticTimerDataType+'",';
      ...
      for (key in listGaugeData) {
        payload += '{"index":{"_index":"'+statsdIndex+'","_type":"doc"}}'+"\n";
        payload += '{';
        innerPayload = '"type":"'+elasticGaugeDataType+'",';
