...
var es_bulk_insert = function elasticsearch_bulk_insert(listCounters, listTimers, listTimerData, listGaugeData) {
      ...
      for (key in listCounters) {
        payload += '{"index":{"_index":"'+statsdIndex+'","_type":"doc"}}'+"\n";
        payload += '{';
        innerPayload = '';
          for (statKey in listCounters[key]){
            if (innerPayload) innerPayload += ',';
            innerPayload += renderKVT(statKey, listCounters[key][statKey], elasticCountType);
          }
      ...
      for (key in listTimers) {
        ...
        innerPayload = '';
          for (statKey in listTimers[key]){
            if (innerPayload) innerPayload += ',';
            innerPayload += renderKVT(statKey, listTimers[key][statKey], elasticTimerType);
          }
        payload += innerPayload +'}'+"\n";
      }
      for (key in listTimerData) {
        ...
        innerPayload = '';
          for (statKey in listTimerData[key]){
            if (innerPayload) innerPayload += ',';
            innerPayload += renderKVT(statKey, listTimerData[key][statKey], elasticTimerDataType);
          }
        payload += innerPayload +'}'+"\n";
      }
      for (key in listGaugeData) {
        ...
        innerPayload = '';
          for (statKey in listGaugeData[key]){
            if (innerPayload) innerPayload += ',';
            innerPayload += renderKVT(statKey, listGaugeData[key][statKey], elasticGaugeDataType);
          }
        payload += innerPayload +'}'+"\n";
      }
      ...
      var optionsPost = {
	    ...
	    ca: elasticCA,
        rejectUnauthorized: elasticCAVerify
      };
      ...
      var req = https.request(optionsPost, function(res) {
          res.on('data', function(d) {
            if (Math.floor(res.statusCode / 100) == 5){
              var errdata = "HTTPS " + res.statusCode + ": " + d;
              lg.log('error', errdata);
            }
          });
      }).on('error', function(err) {
          lg.log('error', 'Error with HTTPS request, no stats flushed.');
          console.log(err);
      });
...
exports.init = function elasticsearch_init(startup_time, config, events, logger) {
  ...
  elasticCA             = configEs.ca             || undefined;
  elasticCAVerify       = configEs.verify_ca      || undefined;
